...
In the table 1 below there is summarized some important statistics about Geo.mean, Volatility and maxTuW variables Firstly, we can see that all variables has 95 observations out of 95 possible, that means that there is no missing values. Secondly the medians and the means of variables are a quite different, especially  for maxTuW, which could mean that there are some skewness in the data, but this will be checked again in the plots below. Lastly we can conclude that the standart deviation of muxTuW is quite large compared with its IQR, so it can tell us that there are a number of outliers in the dataset, because IQR is resistant to outliers, but the standard deviation is not.
...
From scattersplots in Figure \ref{fig:scatter} it seems that there is negative correlation between Geo.mean and Volatility, which means that Geo.mean and Volatility are related to each other, where Geo.mean tends to be higher when Volatility is low. The same couldn't be said about relationship between Geo.mean and maxTuW.  Although there is some negative correlation too between these values, where Geo.mean increases when maxTuW period gets shorter, the relation looks quite flat.

The historgrams in Figure \ref{fig:hist} can tell us that all three variables is skewed. However the Geo.mean variable is the least skewed, but still has some left-skewness. Most of Geo.mean values lie between -0.1 and 0.2. Volatility is right-skewed and the most of values are gathered between 2 and 5. maxTuw is left-skewed, where most of maxTuW values are between 300 and 350.

From the boxplots in Figure  \ref{fig:boxplot}  we can say that maxTuW have few more outliers than Geo.mean and Volatility. Moreover the skewness can also be seen here too, where medians aren't in the middle of IQR. 
...
$$
Y_i = \beta_0 + \beta_1x_{i,1} + \beta_2x_{2,i} + \epsilon_i, \quad \epsilon_i \sim N(0, \sigma^2)
$$
A model states that the geometric avarge rate of return is a function of volatility and maxTuW, where $x_1$ is the volatility and $x_2$ is the maxTuW. Here we assume that the residuals($\epsilon_i$) are independent and identically normally distributed random variables with zero mean and some unknown constant variance.       
...
From the R output above we can conclude:
...
$\hat\beta_0$ is the estimated intercept. The intercept theoretically means that when volatility is 0 and maxTuX is 0 than Geam.mean would be 0.25 with some error.

$\hat\beta_1$ is the estimated effect of volatility and $\hat\beta_2$ is the estimated effect of maxTuW. The effect of volatility is accounted for the effect of maxTuW and viva verca. The negative effect of volatility and maxTuW means that if volatility or maxTuW increases, so Geo.mean decreases, or a lower volatility and a lower maxTuX gives higher Geo.mean.   
...
\colorbox{hightlightColor}{Skal jeg validere for independance?}
...
So the final model,  where $x_1$ is the ¨volatility:
$$
